...
可以取：$R(r)=\dfrac{\dd}{\dd r}r^6(R^6-r^6)=6R^6r^5-12r^{11}$，
此时有$\rho(\symbfit{r})=A(6R^6r^5-12r^{11})Y_{21}(\theta,\phi)$。
此电荷分布的任意阶极矩均为零。

\bigskip

以$\rho(\symbfit{r})=A(6R^6r^5-12r^{11})Y_{21}(\theta,\phi)$为例的一系列电荷构造
...
电势在边界处为零且法向偏导数为零，且函数式显含角量，再代入Poisson方程即可得到电荷分布。（在区域$\Omega$之外，由于$r\to 0,\phi \to 0$，根据静电场唯一性定理
